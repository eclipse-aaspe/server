            ...
            if (aasRegistry != null)
            {
                ...
                if (getRegistry.Count != 0)
                {
                    ...
                    if (submodelRegistryUrl != null)
                    {
                        ...
                        if (AasxCredentials.get(cs.credentials, requestPathGetReg, out _, out _, out _, out var replace) && !string.IsNullOrEmpty(replace))
                        {
                    ...
                    foreach (var greg in getRegistry)
                    {
                        ...
                        if (AasxCredentials.get(cs.credentials, requestPathGetReg, out var queryPara, out userPW, out urlEdcWrapper, out replace) && !string.IsNullOrEmpty(replace))
                        {
                            requestPathGetReg = replace;
                        }
                            ...
                            foreach (var ad in aasDescriptors)
                            {
                                ...
                                if (ad.SubmodelDescriptors != null)
                                {
                                    foreach (var sd in ad.SubmodelDescriptors)
                                    {
                                        ...
                                        if (requestPathGetReg != null && AasxCredentials.get(cList, requestPathGetReg, out queryPara, out userPW, out urlEdcWrapper, out replace))
                                        {
                                            if (replace != "")
                                            {
                                                requestPathGetReg = replace;
