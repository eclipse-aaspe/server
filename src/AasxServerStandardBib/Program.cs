...
namespace AasxServer
{
	...
	public static class Program
	{
		...
		*/
		...
		static Dictionary<string, SampleClient.UASampleClient> OPCClients =
			new Dictionary<string, SampleClient.UASampleClient>();
		...
		public static Dictionary<object, string> generatedQrCodes =
				...
				if (getDirectory)
				{
					...
					for (int j = 0; j < aascount; j++)
					{
						...
						if (Program.env[j] != null)
						{
							...
							aasDescriptor aasDsecritpor = Program.creatAASDescriptor(
								Program.env[j]
							);
					...
					string decriptorData = JsonConvert.SerializeObject(
						descriptortf,
						Formatting.Indented
					);
					...
					try
					{
						...
						foreach (TransmitData td2 in tf2.data)
						{
							...
							if (td2.type == "getaasx" && td2.destination == connectNodeName)
							{
								...
								Byte[] binaryFile = System.IO.File.ReadAllBytes(
									Program.envFileName[aasIndex]
								);
								...
								string fileToken = Jose.JWT.Encode(
									payload,
									enc.GetBytes(
										AasxRestServerLibrary.AasxHttpContextHelper.secretString
									),
									JwsAlgorithm.HS256
								);
													...
													)
													{
														...
														)
														{
															if (tsb.data != null)
															{
																...
																int actualCollections = tsb.data
																	.Value
																	.Count;
																if (
																	actualCollections
							...
							if (td2.type == "submodel")
							{
								foreach (string sm in td2.publish)
								{
									...
									if (aas != null)
									{
										...
										{
											bool toSubscribe =
												Program.submodelsToSubscribe.Contains(existingSm);
											if (!toSubscribe)
											{
						...
						if (sm != null && sm.IdShort != null)
						{
							...
							if (count != 0)
							{
								...
								for (j = 0; j < count; j++)
								{
									...
									string serverNodePrefix = string.Format(
										"ns={0};s=AASROOT.{1}",
										serverNamespaceIdx,
										AASSubmodel
									);
				...
				switch (jp1.Value.Type)
				{
					...
					default:
						...
						try
						{
							if (p.Value == "")
								p.Value = "0";
							string value = (jp1.Value as JValue).ToString(
	...
	public class MySampleServer
	{
		...
		private async void StatusThread()
		{
			while (server != null)
			{
				...
				{
					IList<Opc.Ua.Server.Session> sessions =
