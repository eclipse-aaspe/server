...
#pragma warning restore 8618
	...
	private static async Task<int> Run(CommandLineArguments a)
	{
		...
		{
			var v = System.Environment.GetEnvironmentVariable(ev);
			if (v != null)
			{
				v = v.Replace("\r", "");
				v = v.Replace("\n", "");
	...
	private static int getaasxFile_fileLenBase64 = 0;
	private static int getaasxFile_fileLenBinary = 0;
	private static int getaasxFile_fileTransmitted = 0;
	private static int blockSize = 1500000;

	private static List<TransmitData> tdPending = new() { };
				...
				try
				{
					...
					foreach (var td2 in tf2.data)
					{
						...
						if (td2.type == "getaasx" && td2.destination == connectNodeName)
						{
							...
							var binaryFile = System.IO.File.ReadAllBytes(
						...
						if (td2.type.ToLower().Contains("timeseries"))
						{
							...
							foreach (var smc in AasxTimeSeries.TimeSeries.timeSeriesSubscribe)
							{
								...
								{
									foreach (
										var tsb in AasxTimeSeries.TimeSeries.timeSeriesBlockList
									)
									{
	...
	*/
	...
	private static bool RESTalreadyRunning = false;
	private static long countGetPut = 0;
						...
						if (count != 0)
						{
							...
							lock (opcclientAddLock)
							{
								if (!OPCClients.TryGetValue(URL, out client))
								{
									...
									{
										// make OPC UA client
										client = new SampleClient.UASampleClient(
										                                         URL,
										                                         autoAccept,
										                                         stopTimeout,
										                                         Username,
	...
	)
	{
		...
		catch (ServiceResultException ex)
		{
			Console.WriteLine(
			                  string.Format(
			                                "OPC ServiceResultException ({0}) trying to read {1}",
			                                ex.Message,
			                                clientNodeId.ToString()
			                               )
			                 );
			return;
		}
...
public class MySampleServer
{
	...
	private int serverRunTime = Timeout.Infinite;
	private static bool autoAccept = false;
	private static ExitCode exitCode;
	private static AdminShellPackageEnv[] aasxEnv = null;
