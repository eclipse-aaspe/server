...
{
    public class AssetAdministrationShellServiceDB : IAssetAdministrationShellService
    {
        ...
        public AssetAdministrationShellServiceDB(IAppLogger<AssetAdministrationShellService> logger, IAdminShellPackageEnvironmentService packageEnvService, IMetamodelVerificationService verificationService, ISubmodelService submodelService)
        {
        ...
        public string GetThumbnail(string aasIdentifier, out byte[] byteArray, out long fileSize)
        {
            ...
            if (aas != null)
            {
                if (aas.AssetInformation != null)
                {
                    if (aas.AssetInformation.DefaultThumbnail != null && !string.IsNullOrEmpty(aas.AssetInformation.DefaultThumbnail.Path))
                    {
                        fileName = aas.AssetInformation.GlobalAssetId + aas.AssetInformation.DefaultThumbnail.Path;

                        Stream stream = _database.ReadFile(fileName);
                        byteArray = stream.ToByteArray();
                        fileSize = byteArray.Length;

                        _logger.LogDebug($"Retrived the thumbnail in AAS with Id {aasIdentifier}");
                    }
                    else
                    {
                        throw new NotFoundException($"No default thumbnail embedded in the AssetInformation of the requested AAS.");
