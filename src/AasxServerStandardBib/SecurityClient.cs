...
public class AasxTask
{
                                  ...
                                  string policy = "", string policyRequestedResource = "")
    {
                                ...
                                response = await client.RequestClientCredentialsTokenAsync(new ClientCredentialsTokenRequest
                                                                                           {
                                                                                               ...
                                                                                               ClientAssertion =
                                                                                               {
                                                                                                   Type = OidcConstants.ClientAssertionTypes.JwtBearer, Value = clientLongToken
                                                                                               }
                                                                                           });
                            });
            task.Wait();
        ...
        if (policy != "" && accessToken.Value != "" && clientToken.Value == "")
        {
            ...
            try
            {
                userName = parsed.RootElement.GetProperty("userName").GetString();
            }
            catch
            {
            }
            ...
            try
            {
                expires = parsed.RootElement.GetProperty("exp").GetString();
            }
            catch
            {
            }
                    ...
                    try
                    {
                        if (opName == "get")
                        {
                            if (elementCollection != null)
                            {
                                ...
                                foreach (var text in from jp1 in parsed.EnumerateObject() where jp1.Name == "elem" select jp1.Value.GetRawText())
                                {
                                    receiveCollection = JsonSerializer.Deserialize<SubmodelElementCollection>(
                                                                                                              ...
                                                                                                              new JsonSerializerOptions
                                                                                                              {
                                                                                                                  Converters =
                                                                                                                  {
                                                                                                                      new AdminShellConverters.JsonAasxConverter("modelType",
                                                                                                                       "name")
                                                                                                                  },
                                                                                                                  PropertyNameCaseInsensitive
                                                                                                                      = true // Optional: Make property names case insensitive
                                                                                                              });
