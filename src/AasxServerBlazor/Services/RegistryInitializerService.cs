namespace AasxServerBlazor.Services;
...
public class RegistryInitializerService : IRegistryInitializerService
{
        ...
        if (aasRegistry == null || submodelRegistry == null)
        {
            ...
            if (aasRegistry != null)
            {
                ...
                if (Program.withDb)
                {
                    ...
                    {
                        //TODO (jtikekar, 2025-04-22): to be done, when DB implementation is added
    ...
    void AddAasToRegistry(AdminShellPackageEnv? env, DateTime timestamp, AssetAdministrationShellDescriptor? aasDesc = null)
    {
        ...
        AssetAdministrationShellDescriptor ad;
        ...
        {
            ad = new AssetAdministrationShellDescriptor(aas?.Id);
            var globalAssetId = aas?.AssetInformation?.GlobalAssetId!;

            ad.IdShort = aas?.IdShort;
            var protocolInfo = new ProtocolInformation($"{Program.externalRepository}/shells/{Base64UrlEncoder.Encode(ad.Id)}");
            Console.WriteLine($"AAS {ad.IdShort} {protocolInfo.Href}");
            var endpoint = new Endpoint("AAS-1.0", protocolInfo);
            ad.Endpoints = [endpoint];
            ...
            if (aas?.Submodels != null && aas.Submodels.Count > 0)
            {
                ...
                foreach (var sm in aas.Submodels.Select(smr => env.AasEnv.FindSubmodel(smr)))
                {
                    ...
                    var sd = new SubmodelDescriptor(sm.Id);
                    sd.IdShort = sm.IdShort;
                    var sProtocolInformation = new ProtocolInformation($"{Program.externalRepository}/shells/{Base64UrlEncoder.Encode(ad.Id)}/submodels/{Base64UrlEncoder.Encode(sd.Id)}");
                    var esm = new Endpoint("SUBMODEL-1.0", sProtocolInformation);
